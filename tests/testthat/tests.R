test_that("CV3J equal to HC3 when all clusters are singletons", {
  ...
  expect_equal(vcovHC(feols_fit, type = "HC3"), nobs(feols_fit) / (nobs(feols_fit) - 1) *as.matrix(CR3f))
  
  CR3f <- vcov_CR3J(feols_WLS_fit, cluster = dat$row)
  expect_equal(vcovHC(feols_WLS_fit, type = "HC3"), nobs(feols_WLS_fit) / (nobs(feols_WLS_fit) - 1) *as.matrix(CR3f))
